...
| 11:20 | First EAS Android build (preview profile) after pushing e2e fixes ERRORED at Gradle: Sentry sourcemap-upload task hard-failed with 'organization ID or slug required' since no Sentry credentials exist for this project. Fixed by adding SENTRY_DISABLE_AUTO_UPLOAD=true to all three eas.json build profiles, extending the web build's existing warn-not-block Sentry policy to Android (which never had it). Re-triggering the build. | eas.json | build errored, root-caused via downloaded (brotli-encoded NDJSON) EAS build log, fixed, not yet re-verified | ~25k |
...
| 14:25 | Edited eas.json | expanded (+9 lines) | ~202 |
