...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const tables = [
    ...
    },
    {
      ...
      duration: '30 dk',
      waiter: 'EK',
      items: [{ id: 104, name: 'San Sebastian Cheesecake', qty: 2, price: 195 }],
    },
    {
      id: 'M-08',
      name: 'Masa 08',
  ...
  function switchTab(id) {
    document
      ...
      .forEach((n) => n.classList.toggle('active', n.dataset.tab === id));
    document.querySelectorAll('.panel').forEach((p) => p.classList.toggle('active', p.id === id));
    $('view-title').textContent = titles[id][0];
    $('view-sub').textContent = id === 'tables-view' ? tablesSub() : titles[id][1];
    $('cart-fab').classList.toggle(
      'visible',
      id === 'pos-view' && tableOf(activeTableId).items.length > 0,
    );
  ...
  function renderTables() {
    ...
    $('kpi-occupied-pct').textContent =
      `%${Math.round((active.length / tables.length) * 100)} dolu`;
  ...
  function renderPicker() {
    ...
    tables.forEach((t) => {
      ...
      const dot =
        t.status === 'free' ? 'dot-free' : t.status === 'occupied' ? 'dot-occ' : 'dot-bill';
      const sub =
        t.status === 'free'
  ...
  function openModifier(p) {
    ...
    document
      .querySelectorAll('#mod-cook .opt-btn')
  ...
  const extrasTotal = () =>
    [...document.querySelectorAll('#mod-extras .opt-btn.active')].reduce(
      (s, b) => s + Number(b.dataset.extra),
      0,
    );
  ...
  $('mod-minus').addEventListener('click', () => {
    if (modQty > 1) {
      modQty--;
      $('mod-qty').textContent = modQty;
      updateModTotal();
  ...
  $('mod-plus').addEventListener('click', () => {
    modQty++;
    $('mod-qty').textContent = modQty;
    updateModTotal();
  });
  ...
  $('btn-mod-confirm').addEventListener('click', () => {
    ...
    if (existing) existing.qty += modQty;
      ...
      t.items.push({
        id: modProduct.id,
        name: modProduct.title,
        qty: modQty,
        price: modProduct.price + extrasTotal(),
      });
  ...
  $('btn-kitchen').addEventListener('click', () => {
    const t = tableOf(activeTableId);
    if (t.items.length === 0) {
      toast('Adisyon boş — önce ürün ekleyin', 'ri-error-warning-line');
      return;
    }
  ...
  $('btn-pay').addEventListener('click', () => {
    const t = tableOf(activeTableId);
    if (t.total === 0) {
      toast('Boş adisyon ödenemez', 'ri-error-warning-line');
      return;
    }
    closeSheet('sheet-cart');
    ...
    document
      .querySelectorAll('#split-seg .seg-btn')
  ...
  function renderMenu() {
    ...
    products.forEach((p) => {
      ...
      item.querySelector('.switch').addEventListener('click', (e) => {
        ...
        toast(
          `${p.title} ${p.stock ? 'satışa açıldı' : 'satışa kapatıldı'}`,
          p.stock ? 'ri-check-line' : 'ri-prohibited-line',
        );
      });
  ...
  $('btn-add-item').addEventListener('click', () =>
    toast('Ürün ekleme formu yakında', 'ri-add-line'),
  );
  ...
  document
    ...
    .forEach((btn) => btn.addEventListener('click', () => closeSheet(btn.dataset.close)));
  ...
  $('notif-btn').addEventListener('click', () =>
    toast('3 yeni bildirim (demo)', 'ri-notification-3-line'),
  );
  $('btn-copy-qr').addEventListener('click', () =>
    toast('QR menü bağlantısı kopyalandı', 'ri-file-copy-line'),
  );
  document
