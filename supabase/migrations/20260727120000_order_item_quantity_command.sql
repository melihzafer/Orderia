-- Gönderilmiş sipariş satırında adet değiştirme (kısmi azaltma/artırma).
-- Not komutunun (apply_order_item_note_command) birebir deseni:
-- idempotent client_mutations + base_version kontrolü + audit_events.

create or replace function public.apply_order_item_quantity_command(
...
begin
  ...
  if requested_payload - array['quantity'] <> '{}'::jsonb
    or not (requested_payload ? 'quantity')
    or jsonb_typeof(requested_payload -> 'quantity') is distinct from 'number' then
    raise exception using errcode = '22023', message = 'invalid_order_quantity_payload';
  end if;
  new_quantity := (requested_payload ->> 'quantity')::int;
  if new_quantity < 1 or new_quantity > 999 then
    raise exception using errcode = '22023', message = 'order_quantity_out_of_range';
  end if;
  ...
  if prior_order_item.status = 'cancelled' then
    raise exception using errcode = '22023', message = 'cancelled_item_quantity_is_immutable';
  end if;
  if prior_order_item.status <> 'ordered' then
    raise exception using errcode = '22023', message = 'served_item_quantity_is_immutable';
  end if;
...
revoke execute on function public.apply_order_item_quantity_command(
  uuid,
  uuid,
  uuid,
  uuid,
...
grant execute on function public.apply_order_item_quantity_command(
  uuid,
  uuid,
  uuid,
  uuid,
