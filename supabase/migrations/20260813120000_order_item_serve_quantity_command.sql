-- Kismi servis: bir satirin kac adedinin masaya goturuldugunu saklar.
--
-- `status` satirin tamamini anlatiyordu ("2 kolanin 1'i gitti" anlatilamiyordu).
-- `served_quantity` kismi servisi tasir; `status` ondan turetilir, boylece
-- `status = 'served'` kontrolu yapan mevcut sorgular ve raporlar bozulmaz.
--
-- Ust sinir (`served_quantity <= quantity`) BILEREK bir CHECK kisiti degil:
-- mevcut void/adet komutlari bir satirin adedini dusurebiliyor ve boyle bir
-- kisit onlari ham bir kisit ihlaliyle patlatirdi. Ust sinir asagidaki
-- fonksiyonda ve istemcideki `servedCount()` kirpmasinda uygulanir.

alter table public.order_items
  add column if not exists served_quantity numeric(12, 3) not null default 0
  constraint order_items_served_quantity_check check (served_quantity >= 0);

create or replace function public.apply_order_item_serve_quantity_command(
...
begin
  ...
  if requested_payload is null
    ...
    or requested_payload - array['servedQuantity'] <> '{}'::jsonb
    or jsonb_typeof(requested_payload -> 'servedQuantity') is distinct from 'number' then
    raise exception using errcode = '22023', message = 'invalid_order_item_serve_quantity_payload';
  end if;
  requested_served := (requested_payload ->> 'servedQuantity')::numeric;
  if requested_served < 0 then
    raise exception using errcode = '22023', message = 'invalid_order_item_serve_quantity_payload';
  end if;
  ...
  if prior_order_item.status not in ('ordered', 'served') then
    raise exception using errcode = '22023', message = 'order_item_is_not_servable';
  end if;

  -- Adedin ustune cikmak sessizce kirpilir: istemci de ayni kirpmayi yapiyor
  -- ve satirin adedi arada baska bir cihazdan dusmus olabilir.
  requested_served := least(requested_served, prior_order_item.quantity);
  next_status := case
    when requested_served >= prior_order_item.quantity then 'served'
    else 'ordered'
  end;

  if prior_order_item.served_quantity = requested_served
    and prior_order_item.status = next_status then
    ...
    set served_quantity = requested_served,
        status = next_status,
...
revoke execute on function public.apply_order_item_serve_quantity_command(
  uuid,
  uuid,
  uuid,
  uuid,
...
grant execute on function public.apply_order_item_serve_quantity_command(
  uuid,
  uuid,
  uuid,
  uuid,
